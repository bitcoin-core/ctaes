...
int main(void) {
    ...
    for (p = 0; p < 8; p++) {
        for (i = 0; i < sizeof(ctaes_tests) / sizeof(ctaes_tests[0]); i++) {
            unsigned char key[32], plain[128], cipher[128], ciphered[128], deciphered[128];
            ...
            from_hex(plain + 16 * p, 16, test->plain);
            from_hex(cipher + 16 * p, 16, test->cipher);
            switch (test->keysize) {
                case 128: {
                    AES128_ctx ctx;
                    from_hex(key, 16, test->key);
                    AES128_init(&ctx, key);
                    AES128_encrypt(&ctx, 8, ciphered, plain);
                    AES128_decrypt(&ctx, 8, deciphered, cipher);
                    break;
                }
                case 192: {
                    AES192_ctx ctx;
                    ...
                    AES192_init(&ctx, key);
                    AES192_encrypt(&ctx, 8, ciphered, plain);
                    AES192_decrypt(&ctx, 8, deciphered, cipher);
                    break;
                }
                case 256: {
                    AES256_ctx ctx;
                    ...
                    AES256_init(&ctx, key);
                    AES256_encrypt(&ctx, 8, ciphered, plain);
                    AES256_decrypt(&ctx, 8, deciphered, cipher);
                    break;
                }
            }
            if (memcmp(cipher + 16 * p, ciphered + 16 * p, 16)) {
                fprintf(stderr, "E(p=%i, key=\"%s\", plain=\"%s\") != \"%s\"\n", p, test->key, test->plain, test->cipher);
                fail++;
            }
            if (memcmp(plain + 16 * p, deciphered + 16 * p, 16)) {
                fprintf(stderr, "D(p=%i, key=\"%s\", cipher=\"%s\") != \"%s\"\n", p, test->key, test->cipher, test->plain);
                fail++;
            }
        }
    }
